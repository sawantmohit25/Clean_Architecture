...
const String INVALID_INPUT_FAILURE_MESSAGE='Invalid Input-Number should be a positive integer or zero';
...
class NumberTriviaBloc extends Bloc<NumberTriviaEvent, NumberTriviaState> {
  ...
  Stream<NumberTriviaState> mapEventToState(
    ...
    if(event is GetTriviaForConcreteNumber){
      final inputEither=inputConverter.stringToUnsigneInteger(event.numberString);
  ...
  Stream<NumberTriviaState> _eitherLoadedOrErrorState(Either<Failure,NumberTrivia> failureOrTrivia) async*{
    yield failureOrTrivia.fold((failure)=> Error(message:_mapFailureToMessage(failure)),(trivia)=> Loaded(trivia:trivia));
}
